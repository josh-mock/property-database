...
class PropertyDatabase:
    ...
    def perform_title_search(self, title_number) -> dict:
        try:
            ...
            result = {
                ...
                'OWNERS': filtered_df.apply(lambda row: {
                    'OWNER': row['owner'],
                    'COUNTRY': row['country'],
                    'SOURCE': row['source']
                }, axis=1).tolist()
    ...
    def perform_company_search(self, company) -> dict:
        try:
            ...
            result = {
                'COMPANY': company,
                'COUNTRY': filtered_df['country'].iloc[0],  # Assuming there's at least one row
                'SOURCE': filtered_df['source'].iloc[0],     # Get the first value
